...
export async function POST(request: NextRequest) {
  try {
    ...
    if (productData.code) {
      description = `<p><strong>Code:</strong> ${productData.code}</p>` + description
    }
    if (productData.range) {
      description = `<p><strong>Range:</strong> ${productData.range}</p>` + description
    }

    const formattedProductData = {
      name: productData.title,
      type: "simple",
      regular_price: productData.price.toString().replace(/[^0-9.]/g, ""),
    ...
    if (error.response && error.response.data) {
      ...
      return NextResponse.json({ error: JSON.stringify(error.response.data) }, { status: error.response.status })
    }
